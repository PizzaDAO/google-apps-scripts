 ...
 * Also updates the local crew table with attendance data.
 *
 * @param {Spreadsheet} spreadsheet - The crew spreadsheet
 * @returns {Object} Result object with attendance stats
 */
function takeAttendanceTodayMerge_(spreadsheet) {
  const crewSs = spreadsheet;
 ...
 *
 * @param {Spreadsheet} spreadsheet - The crew spreadsheet
 * @returns {Object} Result from the initial attendance run
 */
function takeAttendanceNowAndScheduleBurst_(spreadsheet) {
  const result = takeAttendanceTodayMerge_(spreadsheet);
  startAttendanceBurst_(spreadsheet);
  return result;
}

/**
 * Starts the attendance burst scheduler.
 */
...
/**
 * Trigger handler for attendance burst.
 * Runs attendance, increments count, stops after configured runs.
 */
function attendanceBurstTick() {
  const props = PropertiesService.getScriptProperties();

  // Find the spreadsheet ID from the stored burst keys
  const allProps = props.getProperties();
  let ssId = null;

  for (const key of Object.keys(allProps)) {
    if (key.startsWith(ATTENDANCE_BURST_COUNT_KEY_PREFIX)) {
      ssId = key.substring(ATTENDANCE_BURST_COUNT_KEY_PREFIX.length);
      break;
    }
  }

  if (!ssId) {
    Logger.log('No active attendance burst found');
  ...
  try {
    ...
    takeAttendanceTodayMerge_(ss);
  } catch (err) {
    Logger.log('attendanceBurstTick error: ' + err.message);
  }
...
/**
 * Stops the attendance burst scheduler.
 */
...
function fetchVoiceAttendance_() {
  ...
  if (code < 200 || code >= 300) {
    throw new Error('Bot fetch failed: HTTP ' + code + ' ' + body);
  }

  const json = JSON.parse(body);
  if (!json || json.ok !== true) {
    throw new Error('Bot returned non-ok: ' + body);
  }
  return json;
}
