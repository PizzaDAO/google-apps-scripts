const { getOAuthDriveApi, getOAuthSheetsApi } = require('./oauth-setup');

const SOURCE_SHEET_ID = '1bRTR85CDHbTAsTG6sVi1jv6H-WjLyf-yvOerLCqeY2Q';

async function copyAndSetupSheet() {
  console.log('Copying pizza-chef sheet for agar.pizza (using OAuth)...\n');

  try {
    const drive = await getOAuthDriveApi();
    const sheets = await getOAuthSheetsApi();
