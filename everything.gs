...
function sendAll(e) {
  ...
  try {
    ...
    ui.alert('Ran: startDiscordEvent + postCrewToDiscord + sendCrewTweet ✅');
  } catch (err) {
    try {
 ...
 * - Removes " Crew" suffix
 ***********************/
function getCrewLookupStringFromActiveSpreadsheet_() {
...
function postCrewToDiscord() {
  ...
  for (let i = 0; i < taskTable.length; i++) {
    ...
    if (stageNorm === 'complete' || stageNorm === 'skipped') continue;
...
function takeAttendanceTodayMerge() {
  const crewSs = SpreadsheetApp.getActiveSpreadsheet();
